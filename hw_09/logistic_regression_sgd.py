...
if __name__ == "__main__":
    ...
    for iteration in range(args.iterations):
        ...
        for batch_x, batch_y in zip(
            ...
            batch_pred = sigmoid(batch_x @ weights)
            batch_grad = np.dot(batch_x.T, batch_pred - batch_y) / args.batch_size
        ...
        train_pred_proba = sigmoid(train_data @ weights)
        train_pred = train_pred_proba >= 0.5

        test_pred_proba = sigmoid(test_data @ weights)
        test_pred = test_pred_proba >= 0.5
        print(
            "After iteration {}: train loss {:.4f} acc {:.1f}%, test loss {:.4f} acc {:.1f}%".format(
                iteration + 1,
                loss(train_pred_proba, train_target),  # The average train loss,
                100 * (train_pred == train_target).mean(),  # Training accuracy,
                loss(test_pred_proba, test_target),  # The average test loss,
                100 * (test_pred == test_target).mean(),  # Test accuracy,
            )
        )
